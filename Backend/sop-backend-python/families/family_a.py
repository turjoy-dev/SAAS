...
Pipeline: Draft -> Critic -> [Edit if needed, single pass only] -> Relint
-> done. Hard cap: exactly one edit pass, no re-critique after editing.
If issues remain after the single edit, they're caught by the free
relint check and logged, never looped on. See RULES.md section 2.
"""

from llm import groq_client, gemini_client
from utils.relint import relint

MAX_EDIT_PASSES = 1  # do not raise this — see hard cap rule
...
def run(manifest: dict, fact_sheet: dict) -> dict:
    ...
    if not critic_result["passed"]:
        ...
        edited = True
        # Deliberately no second gemini_client.critique() call here.
